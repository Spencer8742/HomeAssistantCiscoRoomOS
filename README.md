...
| Button | Refresh meetings | Re-fetches the bookings the device knows about on demand instead of waiting for the 5-minute poll |
...
| Sensor | Next meeting | Title of the next booking (refreshed every 5 minutes, or on demand via the Refresh meetings button). Attributes: the next meeting's start/end time and organizer, plus `meeting_count` and a `meetings` list of every booking the device currently knows about — joinable or not — each flagged with a `joinable` boolean |
